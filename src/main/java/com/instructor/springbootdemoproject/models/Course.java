...
@NamedNativeQuery(name="Course.findStudentCourses",
        query ="select c.id, c.name, c.instructor from course as c join student_courses as sc on c.id = sc.course_id join student as s on s.email = sc.student_email where s.email = :email",
        resultSetMapping = "Mapping.findStudentCourses")
@SqlResultSetMapping(name="Mapping.findStudentCourses",
classes = @ConstructorResult(targetClass = Course.class,
